...
export class SiteMonitor {
    ...
    private defaultTimeout = 15000; // 10 seconds
    ...
    async checkSite(site: SiteConfig): Promise<MonitoringResult> {
        ...
        try {
            const timeoutValue =
                site?.expectedResponseTime ?? this.defaultTimeout;
            console.log(`Monitoring ${site.name} with timeout ${timeoutValue}`);
            const response = await axios.get(site.url, {
                timeout: timeoutValue,
            });

            const responseTime = Date.now() - startTime;
            const expectedTime = timeoutValue;
